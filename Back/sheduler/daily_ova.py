...
fastapi_url =f"{os.getenv('FASTP_API_URL')}/animeOva"

# with open("/test.log", "a") as logfile:
#     logfile.write("Скрипт Обновлению Ova выполнен в: " + str(datetime.now()) + "\n")
...
for year in years:
    ...
    initial_url = f"{base_url}?token={token}&types=anime&with_material_data=true&anime_kind=ova&year={year}"
