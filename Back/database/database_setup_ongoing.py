...
class AnimeSchemaBase(Base):
    __tablename__ = 'anime_schema'
...
class Like(Base):
    ...
    # anime_id = Column(String, ForeignKey('anime_schema.id'), nullable=False)
    anime_id = Column(String, nullable=False)
    is_like = Column(Boolean)
    # anime = relationship('AnimeSchemaBase', back_populates='likes')
    # like_count = Column(Integer, default=0) # Общее количество лайков # Новая колонка
    # dislike_count = Column(Integer, default=0) # Общее количество дизлайков  # Новая колонка
    # view_count = Column(Integer, default=0) # Новая колонка
    title = Column(String, nullable=False)
